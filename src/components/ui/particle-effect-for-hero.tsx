...
export const AntiGravityCanvas: React.FC = () => {
  ...
  const initParticles = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        ...
        color: Math.random() > 0.9 ? '#4285F4' : '#808080', // Changed white to gray for visibility on light mode, blue remains
  ...
  const animate = useCallback((time: number) => {
    ...
    ctx.fillStyle = "#808080"; // Changed to gray
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      ctx.fillStyle = p.color === '#ffffff' 
         ? `rgba(128, 128, 128, ${opacity})` // Changed to gray
         : p.color;
